...
def inference_segmentor(model, img):
    ...
    # encoder_features = model.extract_feat(data['img'][0])
    print('Extracting encoder features + attn weights')
    encoder_features, attn_weights = model.backbone.forward_incl_get_attn_weights(data['img'][0]) # encoder_features, attn_weights

    return result, encoder_features, attn_weights

def get_data(model, img):
    cfg = model.cfg
    device = next(model.parameters()).device  # model device
    # build the data pipeline
    test_pipeline = [LoadImage()] + cfg.data.test.pipeline[1:] # orig
    # test_pipeline = [LoadImage()] + cfg.data.test.pipeline # luca
    test_pipeline = Compose(test_pipeline)
    # print(test_pipeline)
    # prepare data
    data = dict(img=img)
